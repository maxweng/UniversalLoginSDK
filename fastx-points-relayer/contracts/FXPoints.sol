...
contract FXPoints {
    ...
    mapping (uint256 => LockedEth) lockedEths;
    uint256 nextLockedEthID;
    ...
    /**
     * @dev Emit on eth locked to contract
     * @param playerAddr Address of the player who owns the eth
     * @param eth Amount in eth
     * @param ID The identity number of the lock record
     */
    event LockEthEvent(address playerAddr, uint256 eth, uint256 ID);
...
//  _/ |__/||__  | \  |__/ |__/ |__       |  / |__  |__  |  / |__/ |__/  __/
     ...
     */
    function getJackpotProbabilityOfPlayer(address _playerAddr) public view returns (uint256) {
        Jackpot storage _jackpot = game.jackpots[game.jackpotID];
     ...
     * @param _jackpotID The jackpot you want to withdraw
     */
    function withdrawJackpot(uint256 _randNum, uint256 _jackpotID) public {
        address payable _playerAddr = msg.sender;

        Player storage _player = game.players[_playerAddr];
        require(_player.addr != address(0), "You are not a valid player!");

        require(_jackpotID > 0 && _jackpotID <= game.jackpotID, "Invalid jackpot!");
...
//             /
    ...
    */
    ...
    function playRound(address _playerAddr, address payable _aff) public payable {
        ...
        emit LockEthEvent(_playerAddr, _lockedEth.eth, nextLockedEthID++);
    }

    /**
     * @dev Unlock eth and buy keys, the amount according to the game result
     * @param _ID The ID of locked eth record
     * @param _win Means the player win the round, only 10% eth will be used to buy keys
     */
    function finishRound(uint256 _ID, bool _win) public {
        ...
        buyKeysCore(_lockedEth.playerAddr, _eth, _lockedEth.aff);
